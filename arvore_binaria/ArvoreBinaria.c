...
void inicializar(ARVORE_BINARIA *arvore) { arvore = NULL; }

bool vazia(ARVORE_BINARIA *arvore) { return arvore == NULL; }
...
bool adicionarFilho(ITEM item, DIRECAO direcao, ARVORE_BINARIA *arvore) {
  if (vazia(arvore) || (direcao == NoPai) || (direcao == NoRaiz) ||
      existeNo(direcao, arvore))
...
/*void contruirArvoreNotacao(ARVORE_BINARIA *arvore, char notacao[40]) {
  for (int i = 0; i < strlen(notacao); i++) {
    if (strcmp(notacao[i], "(") == 0) {
    }
  }
}
*/
...
bool iguais(ARVORE_BINARIA *arv1, ARVORE_BINARIA *arv2){
  ...
  else if(arv1->esq == NULL && arv1->dir == NULL && arv2->esq == NULL && arv2->dir == NULL){
    return true;
  }
  else if(arv1->esq == NULL && arv2->esq == NULL){
    iguais(arv1->dir,arv2->dir);
  }
  else if(arv1->dir == NULL && arv2->dir == NULL){
